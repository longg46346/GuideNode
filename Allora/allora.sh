...
services:
  inference:
    ...
    healthcheck:
      test: ["CMD", "curl", "-f", "http://localhost:8000/inference/ETH|| exit 1 && curl -f http://localhost:8010/inference/BTC || exit 1 && curl -f http://localhost:8000/inference/SOL || exit 1  && curl -f http://localhost:8000/inference/BNB || exit 1  && curl -f http://localhost:8000/inference/ARB || exit 1"]
      interval: 10s
      timeout: 5s
      retries: 12
    volumes:
